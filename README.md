...
> 当前最新发布：**v1.0.6**（修复设备离线仍显示在线：离线即清理数据库缓存，前端正确判定离线；补全部署访问地址端口说明；v1.0.5 已含运营商真实 logo + 加载卡死修复 + 后台轮询降频）
> 发布时打对应 `vX.Y.Z` tag 并上传 `simadmin-guanli.tar.gz` 资产
...
- **多设备聚合监控**：一台页面管理所有 SimAdmin 设备，卡片展示：
  ...
  - 运行时长、温度、CPU / 内存
  - **OTA 待更新横幅**（有更新时提示）
- **多端同步**：设备配置存服务器 SQLite 数据库（`data/simadmin.db`），任何浏览器/手机访问同一页面看到同一份配置，增删改实时同步
- **短信中心**：
...
- **数据库缓存（低延迟）**：代理从设备抓取的数据写入 SQLite（`data/simadmin.db`）；前端刷新时优先读库，仅在缓存过期时回源打设备，轮询不再每次穿透设备，延迟更低、对设备更友好
- **后台轮询守护（poller）**：`poller.php` 常驻进程每 5 秒（环境变量 `POLLER_INTERVAL` 可调）抓取所有设备的状态与短信写入 SQLite 缓存。这样**即使面板页面未打开**，缓存也始终是最新 ≤5 秒的数据，前端一打开/每次轮询都是低延迟读库；轮询同时预建立设备会话，前端直连更快。由 `simadmin-poller.service`（systemd）托管，崩溃自动重启
- **可切换刷新模式**：顶栏下拉框支持 **5 秒自动刷新 / 30 秒自动刷新 / 手动刷新**；手动模式下仅点「刷新全部」或单设备🔄才回源
- **设备密码可选**：添加设备时密码**可留空**——留空表示该 SimAdmin 未开启密码验证，代理直连无需鉴权；填写密码则表示已开启验证，代理自动登录获取会话
- **自动刷新**：设备状态按所选模式自动刷新（页面隐藏时暂停，静默局部更新不闪屏），离线设备及时发现；短信每 10 秒检测未读（短信走直连，逻辑不变）
...
### 4. 访问
浏览器打开 `http://你的域名或IP:端口/`（端口为宝塔站点端口，默认常为 80/443；未绑定域名时用 `http://服务器IP:宝塔站点端口/`）即可。
...
部署完成后打开 `http://<服务器IP>:<端口>/`（一键脚本默认端口 **5000**），首次访问会要求设置主密码（8–64 位，含两类字符）。
